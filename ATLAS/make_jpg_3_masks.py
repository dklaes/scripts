...
for field in field_list:
    ...
    mask_star_reg2_f = '%s_%s_stars_11_9.reg' % (field, band,)
    mask_star_reg2 = os.path.join(image_dir, field, band, masks, mask_star_reg2_f)
    mask_star_reg3_f = '%s_%s_stars_12_10.reg' % (field, band,)
    mask_star_reg3 = os.path.join(image_dir, field, band, masks, mask_star_reg3_f)
    ...
    S.call(shcommand, shell=True)  # ww doesn't like the all-cap POLYGON file
    shcommand = 'sed s/POLYGON/polygon/g %s > tmp3_%s' % (mask_star_reg3, mask_star_reg3_f)
    S.call(shcommand, shell=True)  # ww doesn't like the all-cap POLYGON file
    ...
    mask_fits3 = '%s/%s_mask3.fits' % (final_mask_dir, field)
    coadd_mask_fits3 = '%s/%s_coadd_mask3.fits' % (final_mask_dir, field)
    ...
    shcommand += ' -WEIGHT_MAX 100000.0'  # maximum weight in RCS2 single exposures ~250
    shcommand += ' -FLAG_NAMES "blank.fits"'        # we have weight fits file instead (don't need blank.fits)
    shcommand += ' -POLY_NAMES %s' % (mask_star_reg3,)
    shcommand += ' -POLY_OUTFLAGS 1'
    shcommand += ' -VERBOSE_TYPE FULL'
    shcommand += ' -OUTFLAG_NAME %s' % (mask_fits3,)
    shcommand += ' -OUTWEIGHT_NAME ""'
    shcommand += ' -WEIGHT_OUTFLAGS 1'
    shcommand += ' -FLAG_WMASKS 0xff'
    shcommand += ' -FLAG_MASKS 0x01'
...
#    S.Popen(shcommand, shell=True)
    ...
    print shcommand
    S.call(shcommand, shell=True)
    shcommand = "ic '%s %s +' %s %s > %s" % (a1, a2, coadd, mask_fits3, coadd_mask_fits3,)
    print shcommand
    S.call(shcommand, shell=True)

    # Make the colored image.
    stiff_var = ' -BINNING 8'
    shcommand = '%s %s -OUTFILE_NAME %s %s %s %s' % (stiff, stiff_var, tiff, coadd_mask_fits1, coadd_mask_fits2, coadd_mask_fits3)
    S.call(shcommand, shell=True)
